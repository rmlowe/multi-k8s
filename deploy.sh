...
docker build -t rmlowe/multi-worker:latest -t rmlowe/multi-worker:$SHA -f ./worker/Dockerfile ./worker
...
kubectl set image deployments/worker-deployment worker=rmlowe/multi-worker:$SHA